...
- Explicit legacy/v0 JSON support. V1, unknown versions and configuration-bearing payloads are rejected before resource interpretation. An omitted version is labelled unspecified rather than silently declared legacy. This is safe unsupported handling, not implemented v1 decoding.
...
- One-based displayed outer positions and not-reached status derived from `meta.err.InstructionError`. Completed prior instructions do not imply committed state in a failed transaction.
